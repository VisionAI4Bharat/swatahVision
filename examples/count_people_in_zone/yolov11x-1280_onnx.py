...
def detect(
    frame: np.ndarray, model: any, confidence_threshold: float = 0.5
) -> sv.Detections:
    """
    ...
    """
    results = model(frame)
    detections = sv.Detections.from_yolo(results)
...
if __name__ == "__main__":
    ...
    model = sv.Model(model=args.source_weights_path, engine=sv.Engine.ONNX, hardware=sv.Hardware.GPU)
